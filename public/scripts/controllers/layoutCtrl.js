'use strict';
/*global mainApp, moment, angular, window, parent*/
/*jslint unparam:true*/
mainApp.controller('LayoutCtrl', ['$scope', '$timeout', 'pagesService', 'websiteService', 'postsService', 'userService', 'accountService', 'ENV', '$window', '$location', '$route', '$routeParams', '$filter', '$document', '$anchorScroll', '$sce', 'postService', 'paymentService', 'productService', 'courseService', 'ipCookie', '$q', 'customerService', 'pageService', 'analyticsService', 'leafletData', 'orderService',
  function ($scope, $timeout, pagesService, websiteService, postsService, userService, accountService, ENV, $window, $location, $route, $routeParams, $filter, $document, $anchorScroll, $sce, PostService, PaymentService, ProductService, CourseService, ipCookie, $q, customerService, pageService, analyticsService, leafletData, OrderService) {
    ...
    $scope.getCourse = function (campaignId) {
      _.each(that.courses, function (course) {
        if (course._id === parseInt(campaignId, 10)) {
          return course;
        }
      });
    };
      ...
      if (err) {
        ...
        if (data) {
          ...
          for (k; k < data.length; k++) {
            if (data[k].post_category) {
              if (that.categories.indexOf(data[k].post_category) <= -1) {
                that.categories.push(data[k].post_category);
              }
            }
          }
          ...
          that.latestposts = [];
          var l = 0;
          for (l; i < data.length; l++) {
            that.latestposts.push(data[l]);
          }
    ...
    $scope.saveBlogData = function (iframe) {
      if (iframe) {
        ...
        _.each(posts, function (post) {
          var blog_id = post.attributes['data-id'].value;
          var post_excerpt_div = post.querySelectorAll('.post_excerpt');
          var post_title_div = post.querySelectorAll('.post_title');
