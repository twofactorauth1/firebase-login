...
angular.module('mainApp')
    ...
    .directive('mediaModal', [ 'FileUploader', 'AssetService', '$http', '$timeout',function (FileUploader, AssetService, $http, $timeout) {
        return {
            require: [],
            restrict: 'C',
            transclude: false,
            ...
            controller: function ($scope, AssetService,  $compile) {
                ...
                $http.get('/views/partials/mediamodal.html').success(function(data){
                ...
                $scope.m.deleteAsset = function (assetId) {
                    AssetService.deleteAssetById(function (resp, status) {
                        if (status === 1) {
                            $scope.assets.forEach(function (v, i) {
                                if (v._id === assetId) {
                ...
                $scope.m.deleteAsset = function () {
                    AssetService.deleteAssetById($scope.batch, function (assetId, resp, status) {
                        if ( status === 1 ) {
                            $scope.assets.forEach(function (v, i) {
                                if (v._id === assetId) {
                                    $scope.assets.splice(i, 1);
            ...
            link: function (scope, element) {
                scope.assets = [];
                AssetService.getAssetsByAccount(function(data){
                    scope.assets = data;
                });
