...
app.controller('AnonymousContactDetailsController', anonymousContactDetailsController);

anonymousContactDetailsController.$inject = ['$scope', '$state', '$window', '$modal', '$stateParams', '$attrs', '$filter', '$document', '$timeout', 'toaster', 'ContactService', 'CommonService', 'OrganizationService'];
/* @ngInject */
function anonymousContactDetailsController($scope, $state, $window, $modal, $stateParams, $attrs, $filter, $document, $timeout, toaster, ContactService, CommonService, OrganizationService) {

    console.info('anonymous contact-details directive init...')

    var vm = this;

    vm.init = init;    
    vm.state ={
    	fingerprintId: $stateParams.fingerprintId,
    	location: {},
    	ip_geo_address: '',
