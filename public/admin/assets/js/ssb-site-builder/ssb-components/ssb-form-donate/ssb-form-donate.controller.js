/*global app,console ,angular ,window, $, moment ,document ,Fingerprint ,PAYPAL ,_*/
...
(function () {
	...
	function ssbFormDonateComponentController($scope, $attrs, $filter, $transclude, $injector, formValidations, $timeout, $sce, $location, $interval, ENV, $modal) {
		...
		var productService = null,
			...
			$routeParams = null,
			orderCookieData = null,
			orderCookieKey = 'order_cookie',
			vm = this,
			showErrorMessage = function (inputId, errorMessage) {
				$(".donation-v1 #" + inputId + " .error").html(errorMessage);
				$(".donation-v1 #" + inputId).addClass('has-error');
				$(".donation-v1 #" + inputId + " .glyphicon").addClass('glyphicon-remove');
			},
			removeErrorMessage = function (inputId) {
				$(".donation-v1 #" + inputId + " .error").html("");
				$(".donation-v1 #" + inputId).removeClass('has-error').addClass('has-success');
				$(".donation-v1 #" + inputId + " .glyphicon").removeClass('glyphicon-remove').addClass('glyphicon-ok');
			};
		...
		vm.createUser = function (form) {
			...
			var ipCookie,
				userService,
				skipWelcomeEmail,
				donateFormCampaignId,
				sendEmailId,
				donateFormCampaignTags,
				fingerprint,
				sessionId,
				first_name = "",
				last_name = "";
			...
			if (!vm.component.campaignId || sendEmailId) {
				...
				donateFormCampaignId = vm.component.campaignId;
			}

			donateFormCampaignTags = [];
			if (vm._campaignObj && angular.isDefined(vm._campaignObj.searchTags) && vm._campaignObj.searchTags.tags.length) {
				donateFormCampaignTags = _.uniq(_.pluck(vm._campaignObj.searchTags.tags, 'data'));
			}
			...
			var formatted = {
				...
				campaignId: donateFormCampaignId,
				campaignTags: donateFormCampaignTags,
		...
		function _formattedOrder() {
			...
			if (vm.product.tags) {
				vm.product.tags = _.map(vm.product.tags, function (tag) {
					return tag.toLowerCase();
				});
			}

			if (vm.component.tags) {
				vm.component.tags = _.map(vm.component.tags, function (tag) {
					return tag.toLowerCase();
				});
			}
			...
			var url = $location.absUrl().split('?')[0],
				_donationAmount = vm.formBuilder.amount,
				order = {
				...
				totalAmount = parseFloat(vm.formBuilder.amount),
				_item = {
		...
		function checkCardName() {
			...
			if (!name) {
				showErrorMessage("card_name", "Card Name Required");
			} else {
		...
		function checkCardExpiry() {
			...
			if (!expiry || !exp_month || !exp_year) {
				var expError;
				if (!expiry) {
					expError = "Expiry Required.";
				} else if (!exp_month) {
					expError = "Expiry Month Required.";
				} else if (!exp_year) {
					expError = "Expiry Year Required.";
				}
				showErrorMessage("card_expiry", expError);
				vm.isCardExpired = true;
			} else {
				vm.yearLength = exp_year.length;
				if (vm.yearLength == 2) {
					if (parseInt(exp_year) < parseInt(vm.currentYear)) {
						showErrorMessage("card_expiry", "Card Year has Expired.");
						vm.isCardExpired = true;
						return;
					}
					...
					showErrorMessage("card_expiry", "Card Year is not valid.");
					vm.isCardExpired = true;
					return;
				}

				if (exp_month < vm.currentMonth && parseInt(exp_year) <= vm.currentYear) {
					showErrorMessage("card_expiry", "Card Month has Expired");
					vm.isCardExpired = true;
				} else if (exp_month > 12) {
					showErrorMessage("card_expiry", "Card Month is invalid");
					vm.isCardExpired = true;
				} else {
						...
						if (error) {
							switch (error.param) {
								...
								case "cvc":
									...
									break;
								case "name":
									showErrorMessage("card_name", error.message);
							}
							vm.checkoutModalState = 4;
							return;
						}
		...
		function getCredentials() {
			if ($injector.has('accountService')) {
				...
				accountService(function (err, account) {
					...
					account.credentials.forEach(function (cred) {
						if (cred.type == 'stripe') {
							vm.stripeInfo = cred;
						}
					});
		...
		function setInitialCheckoutState() {
			if ($routeParams && $routeParams.state && $routeParams.comp == 'donation') {
				...
				if (vm.checkoutModalState == 5 && orderCookieData && $injector.has('orderService')) {
					var orderService = $injector.get('orderService');
				...
				vm.showPaypalErrorMsg = (vm.checkoutModalState == 2);
			}
		}

		function checkDateValidityFn() {
			if (!angular.isDefined(vm.component.productSettings.timePeriod) || !vm.component.productSettings.timePeriod.enabled) {
				return true;
			} else if ($('#form-donate-modal-amount-' + vm.component._id).hasClass('ng-invalid')) {
				return false;
			} else if (vm.component.productSettings.timePeriod.startDate && vm.component.productSettings.timePeriod.endDate) {
		...
		function getCampaings() {
			if (vm.component.campaignId && $injector.has("campaignService")) {
				var campaignService = $injector.get("campaignService");
