'use strict';
/*global app*/
app.controller('SiteBuilderAddSectionModalController', ['$scope', '$document', '$modalInstance', '$timeout', 'WebsiteService', 'toaster', 'components', 'clickedIndex', 'isEmail', 'pageHandle', function ($scope, $document, $modalInstance, $timeout, WebsiteService, toaster, components, clickedIndex, isEmail, pageHandle) {
  ...
  vm.addComponent = function (addedType) {
    if (!vm.saveLoading) {
      vm.saveLoading = true;
      ...
      if (addedType.type === 'footer' || addedType.type === 'navigation' || addedType.type === 'single-post' || addedType.type === 'blog-teaser' || addedType.type === 'blog') {
        componentType = _.findWhere(vm.components, {
          type: addedType.type
        });
        if (componentType) {
      ...
      WebsiteService.getComponent(addedType, addedType.version || 1, function (newComponent) {
        if (newComponent) {          
          vm.closeModal();
          vm.components.splice(vm.clickedIndex + 1, 0, newComponent);
          $timeout(function () {          
  ...
  if (vm.isEmail) {
    ...
    var checkIfFooterExists = _.findWhere(vm.components, {
      type: 'footer'
    });
    if(!checkIfFooterExists)
    {
      var footerComponent = _.findWhere(vm.componentTypes, {
        type: 'footer'
      });
      footerComponent.enabled = true;
    }
    if(vm.pageHandle == 'single-post')
    {
      var checkIfPostExists = _.findWhere(vm.components, {
        type: 'single-post'
      });
      if(!checkIfPostExists)
      {
        var postComponent = _.findWhere(vm.componentTypes, {
          type: 'single-post'
        });
