    ...
    function indiEmailBuilderController($scope, $rootScope, EmailBuilderService, EmailCampaignService, SimpleSiteBuilderService, $stateParams, $state, toaster, AccountService, WebsiteService, $modal, $timeout, $document, $window, $location, SweetAlert) {
        ...
        function saveFn() {
            if (vm.checkSettingsValidityFn()) {
                ...
                vm.uiState.dataLoaded = false;
                console.log('---------------vm.state.email-----------',vm.state.email);
        ...
        function pageChanged(originalEmail, currentEmail) {
            console.log(angular.equals(originalEmail, currentEmail),'-----difff---------',vm.state.pendingEmailChanges);
            ...
            else if(angular.equals(originalEmail, currentEmail)  === true && vm.state.pendingEmailChanges === true || angular.equals(originalEmail, currentEmail)  === true && vm.state.pendingEmailChanges === false){
        ...
        function isEmptyStyleAdded(diff1, diff2) {
            if(diff1 &&
                    diff2 &&
                    angular.isDefined(diff1) && angular.isDefined(diff2) && diff1.replace && diff2.replace)
                {
