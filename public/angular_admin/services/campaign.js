    ...
    app.register.service('CampaignService', ['$http', 'ToasterService', function($http, ToasterService) {
          var baseUrl = '/api/1.0/';
            ...
            this.postCampaign = function(campaign, fn) {
                ...
                var apiUrl = baseUrl + ['campaign', 'campaign'].join('/');
                $http({
                    url: apiUrl,
                    method: "POST",
                  ...
                  .error(function(error) {
                    console.error('CampaignService: postCampaign error >>> ', error);
                  });
            };

            this.getCampaign = function(campaignId, fn) {

                var apiUrl = baseUrl + ['campaign', 'campaign', campaignId].join('/');
                $http({
                    url: apiUrl,
                    method: "GET"
                  })
            ...
            this.getCampaigns = function(fn) {
                ...
                var apiUrl = baseUrl + ['campaign', 'campaigns'].join('/');
                $http({
                    url: apiUrl,
                    method: "GET"
                  })
                  ...
                  .error(function(error) {
                    console.error('getCampaigns: getCampaigns error >>> ', error);
                  });
            };
