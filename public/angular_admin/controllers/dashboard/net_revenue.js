define(['app', 'productService', 'paymentService','ngProgress', 'formatCurrency', 'highcharts', 'highcharts-ng'], function(app) {
    app.register.controller('NetRevenueCtrl', ['$scope', 'ProductService', 'PaymentService', 'ngProgress', function($scope, ProductService, PaymentService, ngProgress) {
        ngProgress.start();
        $scope.$back = function() {
            window.history.back();
        };
        ...
        Keen.ready(function() {
            ...
            $scope.client.run([netRevenueThisMonth, netRevenueDayPreviousMonth, netRevenueLastSixty, netRevenuePreviousSixty, netRevenuePreviousNinety, netRevenuePreviousYear], function(response) {
                $scope.thisMonth = this.data[0].result / 100;
                $scope.previousMonth = this.data[0].result / 100;
                $scope.lastSixty = this.data[2].result / 100;
                $scope.previousSixty = this.data[2].result / 100;
                ...
                console.log('previousSixty >>> ', $scope.previousSixty);
                console.log('lastSixty >>> ', $scope.lastSixty);
            ...
            $scope.client.run(revSeries, function(response){
                ...
                console.log('result >>> ', result[0].timeframe.start);
                var lineData = [];
                var startDate;
            ...
            $scope.client.run([revenueByPlans, customerByPlans], function(response){
                console.log('revenueByPlans >>> ', this.data[0].result);
