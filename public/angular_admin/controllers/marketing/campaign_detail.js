        ...
        $scope.initializeJSPlumb = function() {
            jsPlumb.ready(function() {
                console.log('plumb ready >>>');
                    ...
                    $scope.addStep = function(type, id) {
                        var step2add;
                        if (type === 'email') {
                            step2add = {
                                "type": "email",
                        ...
                        console.log("#" + id);
                        var newWidget = $("#" + id).clone().appendTo(".flowchart-demo");
                        newWidget.attr('id', 'pageWidget' + windowLength);
                        instance.draggable('pageWidget' + windowLength, {
                            grid: [1, 1]
                        });
                        _addEndpoints(newWidget.attr('id'), ["RightMiddle"], ["LeftMiddle"]);
                        $("#" + id).fadeOut();
                        setTimeout(function() {
                            $("#" + id).css({
                                'top': '',
                    ...
                    jsPlumb.draggable($("#newPageWidget"), {
                        ...
                        stop: function(params) {
                            $scope.addStep(params.el.getAttribute("data-type"), params.el.id);
                        }
                    });
                    ...
                    instance.doWhileSuspended(function() {
                        ...
                        for (var i = 0; i < $scope.campaign.steps.length; i++) {
                            console.log('$scope.campaign.steps[i]._id ', $scope.campaign.steps[i]._id);
                            _addEndpoints($scope.campaign.steps[i]._id, ["RightMiddle"], ["LeftMiddle"]);
                        };
