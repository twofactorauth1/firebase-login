define(['app', 'customerService', 'stateNavDirective', 'truncateDirective', 'ngProgress', 'headroom', 'ngHeadroom', 'toasterService', 'iStartsWithFilter', 'ngInfiniteScroll', 'scrollerDirective', 'userService'], function(app) {
    app.register.controller('CustomerCtrl', ['$scope', 'CustomerService', 'ngProgress', 'ToasterService', '$window', '$filter', 'UserService', function($scope, CustomerService, ngProgress, ToasterService, $window, $filter, UserService) {
        ...
        $scope.orderByFn = function() {
            $scope.fetchedCustomers = $filter('orderBy')($scope.fetchedCustomers, $scope.customerOrder);
            $scope.renderedCustomers = $filter('orderBy')($scope.renderedCustomers, $scope.customerOrder);
        };
        ...
        CustomerService.getCustomersShortForm(fetchFields, function(customers) {
            ...
            $scope.$watch('changeDisplayFormat', function(newValue, oldValue) {
                if (newValue && oldValue !== undefined) {
                    $scope.userPreferences.customerSettings.customerDisplayFormat = newValue;
            ...
            $scope.$watch('sortOrder', function(newValue, oldValue) {
                if (newValue && $scope.userPreferences && $scope.userPreferences.customerSettings && oldValue !== undefined) {
                    $scope.userPreferences.customerSettings.customerOrder = newValue;
                    $scope.savePreferencesFn();
                }
            });
            ...
            $scope.gridViewDisplay = function(value) {
                $scope.listgrid = value;
            ...
            UserService.getUserPreferences(function(preferences) {
                ...
                if (customerSettings) {
                    ...
                    $scope.sortOrder = $scope.userPreferences.customerSettings.customerOrder;
                    $scope.changeDisplayFormat = $scope.userPreferences.customerSettings.customerDisplayFormat;
                    ...
                    $scope.listgrid = $scope.userPreferences.customerSettings.gridViewDisplay;
                } else {
                    $scope.userPreferences.customerSettings = {
                        customerOrder: 1,
                        customerDisplayFormat: 1,
