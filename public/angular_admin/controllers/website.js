define([
    ...
    app.register.controller('WebsiteCtrl', [
        ...
        function($scope, $window, $timeout, $location, WebsiteService, UserService, toaster, ngProgress, $rootScope, CourseService, NavigationService, SweetAlert, blockUI) {
            ...
            //revert the page to historical page version
            $scope.revertPage = function(version) {
                var versionId = version._id;
                var split = versionId.split("_");
                WebsiteService.revertPageVersion($scope.currentPage._id, split[1], function(revertedPage) {
                    $scope.currentPage = revertedPage;
                    $scope.components = $scope.currentPage.components;
                    $scope.updateIframeComponents();
                });
            };

            //show the preview section with screenshot
            $scope.showPreview = function(version) {
                ...
                if (version.screenshot) {
                    ...
                    img.addEventListener('load', function() {
                        ...
                        if (realSize.naturalWidth == 1280 && realSize.naturalHeight == 512) {
                            $scope.$apply(function() {
                                $scope.selectedVersion.screenshot = null;
                            });
                        }
                    });
                }
            ...
            //hide the preview section with screenshot
            $scope.hidePreview = function() {
                $scope.selectedVersion = '';
                $scope.previewSwitch = false;
            };

            /***** End Historical Component *****/
            ...
            $scope.updatePage = function(handle, editing) {
                ...
                WebsiteService.getPages($scope.account.website.websiteId, function(pages) {
                    ...
                    WebsiteService.getPageVersions($scope.currentPage._id, function(pageVersions) {
                        ...
                        //format dates 
                        for (var i = 0; i < pageVersions.length; i++) {
                            var seconds = moment(pageVersions[i].modified.date).format();
                            pageVersions[i].modified.date = seconds
                        };
