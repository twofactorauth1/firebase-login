...
(function (factory) {
  ...
  $.FE.PLUGINS.image = function (editor) {
    ...
    /**
     * Insert image using media manager callback.
     */

    function insertByMediaCallback (url, current_image) {
      editor.selection.restore();
      var $popup = editor.popups.get('image.insert');
      var $current_image = current_image;
      showProgressBar();
      _setProgressMessage('Loading image');
      insert(url, true, [], $current_image);
    }
    ...
    function _initInsertPopup (delayed) {
      ...
      var mediaIndex = editor.opts.imageInsertButtons.indexOf('mediaManager');
      ...
      if (mediaIndex >= 0) {
        active = ' fr-active';
        if (urlIndex >= 0 && mediaIndex > urlIndex) {
          active = '';
        }

        media_layer = '<div data-cmd="insertMedia" class="fr-image-upload-media-layer' + active + ' fr-layer" id="fr-image-upload-media-layer-' + editor.id + '"><button type="button" class="fr-command fr-submit" data-cmd="insertMedia"><i class="ti-upload"></i><span>Media Manager</span></button></div>';
      }
    ...
    return {
      ...
      refreshByMedia: refreshByMedia,
      upload: upload,
      insertByURL: insertByURL,
      insertByMediaCallback: insertByMediaCallback,
      align: align,
  ...
  // Insert image button inside the insert by URL layer.
  $.FE.RegisterCommand('insertMedia', {
    title: 'Insert Image',
    undo: true,
    callback: function() {
      var ed = this;
      ...
      clickandInsertImageButton(this, image, this.image.insertByMediaCallback);
