...
(function (factory) {
  ...
  $.FE.PLUGINS.link = function (editor) {
    ...
    function _initInsertPopup (delayed) {
      ...
      if (!editor.opts.linkAlwaysBlank) {
        input_layer += '<div class="fr-checkbox-line"><span class="fr-checkbox"><input data-cmd="updateLinkTargetBlank" name="target" class="fr-command fr-link-attr" data-checked="_blank" type="checkbox" id="fr-link-target-' + editor.id + '" tabIndex="' + (++tab_idx) + '"><span>' + checkmark + '</span></span><label class="fr-command" data-cmd="updateLinkTargetBlank" for="fr-link-target-' + editor.id + '">' + editor.language.translate('Open in new tab') + '</label></div>';
        input_layer += '<div class="fr-checkbox-line"><span class="fr-checkbox"><input data-cmd="updateLinkTargetSelf" name="target_self" class="fr-command fr-link-attr" data-checked="_self" type="checkbox" id="fr-link-target-self' + editor.id + '" tabIndex="' + (++tab_idx) + '"><span>' + checkmark + '</span></span><label class="fr-command" data-cmd="updateLinkTargetSelf" for="fr-link-target-self' + editor.id + '">' + editor.language.translate('Reload page') + '</label></div>';
      }
    ...
    function insert (href, text, attrs) {
      ...
      if(attrs.target_self){
        attrs.target = '_self';
      }
      delete attrs.target_self;
  ...
  $.FE.RegisterCommand('updateLinkTargetSelf', {
    undo: false,
    focus: false,
    callback: function (cmd, val) {
  ...
  $.FE.RegisterCommand('updateLinkTargetBlank', {
    undo: false,
    focus: false,
    callback: function (cmd, val) {
      var $popup = this.popups.get('link.insert');
      $popup.find('input[name="target_self"].fr-link-attr[type="checkbox"]').prop('checked', false);
    }
  })
