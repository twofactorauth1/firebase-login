...
(function (factory) {
  ...
  $.FE.PLUGINS.video = function (editor) {
    ...
    function _initInsertPopup (delayed) {
      ...
      // Video media upload layer.
      var media_layer = '';
      if (editor.opts.videoInsertButtons.indexOf('videoMediaManager') >= 0) {
        media_layer = '<div class="fr-video-upload-media-layer fr-layer" id="fr-video-upload-media-layer-' + editor.id + '"><button type="button" class="fr-command fr-submit" data-cmd="insertVideoMedia"><i class="ti-upload"></i><span>Media Manager</span></button></div>';
      }
    ...
    function refreshByMedia ($btn) {
      var $popup = editor.popups.get('video.insert');
      if ($popup.find('.fr-video-upload-media-layer').hasClass('fr-active')) {
    ...
    return {
      ...
      insertByMediaAsset: insertByMediaAsset,
  ...
  // Insert video button inside the insert by URL layer.
  $.FE.RegisterCommand('insertVideoMedia', {
    title: 'Insert Video',
    undo: true,
    callback: function() {
      var ed = this;
      ...
      this.selection.save();
      var video = this.video.get();
      clickandInsertVideoButton(this, video, this.video.insertByMediaAsset);
    }
  })
