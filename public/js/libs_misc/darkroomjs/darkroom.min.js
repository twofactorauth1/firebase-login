! function(window, document, fabric) {
    ...
    function Plugin(darkroom, options) {
        this.darkroom = darkroom, this.options = extend(options, this.defaults), this.initialize()
    }

    function Toolbar(element) {
        this.element = element, this.actionsElement = element.querySelector(".darkroom-toolbar-actions")
    }
    ...
    window.Darkroom = Darkroom, Darkroom.plugins = [], Plugin.prototype = {
        defaults: {},
        initialize: function() {}
    ...
    }, Darkroom.Plugin = Plugin, Toolbar.prototype.createButtonGroup = function() {
        var buttonGroup = document.createElement("li");
        return buttonGroup.className = "darkroom-button-group", this.actionsElement.appendChild(buttonGroup), new ButtonGroup(buttonGroup)
        ...
        button.className = "darkroom-button darkroom-button-" + options.type, button.innerHTML = '<i class="darkroom-icon-' + options.image + '"></i>', this.element.appendChild(button);
    ...
    Darkroom.prototype = {
        ...
        init: function(element, options, plugins) {
            ...
            if (this.options = extend(options, this.defaults), "string" == typeof element && (element = document.querySelector(element)), null !== element) {
                ...
                image.onload = function() {
                    _this.createFabricImage(element).initDOM(element).initPlugins(plugins), _this.options.init.bind(_this).call()
                }, image.src = element.src
            }
        },
        initDOM: function(element) {
            var toolbar = document.createElement("div");
            toolbar.className = "darkroom-toolbar", toolbar.innerHTML = '<ul class="darkroom-toolbar-actions"></ul>';
            var canvas = document.createElement("canvas"),
                canvasContainer = document.createElement("div");
            return canvasContainer.className = "darkroom-image-container", canvasContainer.appendChild(canvas), this.container = document.createElement("div"), this.container.className = "darkroom-container", this.container.appendChild(toolbar), this.container.appendChild(canvasContainer), element.parentNode.replaceChild(this.container, element), this.toolbar = new Toolbar(toolbar), this.canvas = new Canvas(canvas, {
                selection: !1,
                backgroundColor: "#ccc"
            }), this.canvas.setWidth(this.image.getWidth()), this.canvas.setHeight(this.image.getHeight()), this.canvas.add(this.image), this.canvas.centerObject(this.image), this.image.setCoords(), this
        },
        createFabricImage: function(imgElement) {
            ...
            return width *= scale, height *= scale, this.image = new fabric.Image(imgElement, {
                ...
                hasBorders: !1
							...
							}), this.image.setScaleX(scale), this.image.setScaleY(scale), this
        },
        initPlugins: function(plugins) {
            ...
            for (var name in plugins) {
                ...
                options !== !1 && (this.plugins[name] = new plugin(this, options))
            }
        },
...
function(window, document, Darkroom, fabric) {
    ...
    Darkroom.plugins.history = Darkroom.Plugin.extend({
        initialize: function() {
            this._initButtons(), this.backHistoryStack = [], this.forwardHistoryStack = [], this._snapshotImage(), this.darkroom.addEventListener("image:change", this._onImageChange.bind(this))
        },
        goBack: function() {
            0 !== this.backHistoryStack.length && (this.forwardHistoryStack.push(this.currentImage), this.currentImage = this.backHistoryStack.pop(), this._applyImage(this.currentImage), this._updateButtons())
        },
        goForward: function() {
            0 !== this.forwardHistoryStack.length && (this.backHistoryStack.push(this.currentImage), this.currentImage = this.forwardHistoryStack.pop(), this._applyImage(this.currentImage), this._updateButtons())
        },
        _initButtons: function() {
            ...
            return this.backButton = buttonGroup.createButton({
                ...
                image: "forward",
                disabled: !0
            }), this.backButton.addEventListener("click", this.goBack.bind(this)), this.forwardButton.addEventListener("click", this.goForward.bind(this)), this
        },
        _updateButtons: function() {
            this.backButton.disable(0 === this.backHistoryStack.length), this.forwardButton.disable(0 === this.forwardHistoryStack.length)
        },
        _snapshotImage: function() {
            var image = new Image;
            image.src = this.darkroom.snapshotImage(), this.currentImage = image
        },
        _onImageChange: function() {
            this.backHistoryStack.push(this.currentImage), this._snapshotImage(), this.forwardHistoryStack.length = 0, this._updateButtons()
        },
        _applyImage: function(image) {
            ...
            canvas.setWidth(image.width), canvas.setHeight(image.height), this.darkroom.image.remove(), this.darkroom.image = imgInstance, canvas.add(imgInstance)
        }
    })
...
function(window, document, Darkroom) {
    ...
    Darkroom.plugins.rotate = Darkroom.Plugin.extend({
        ...
        rotate: function(angle) {
            ...
            var width, height;
            Math.abs(angle) % 180 ? (height = image.getWidth(), width = image.getHeight()) : (width = image.getWidth(), height = image.getHeight()), canvas.setWidth(width), canvas.setHeight(height), image.rotate(angle), canvas.centerObject(image), image.setCoords(), canvas.renderAll(), darkroom.dispatchEvent("image:change")
        }
    })
}(window, document, Darkroom, fabric),
function(window, document, Darkroom, fabric) {
                ...
                image.onload = function() {
                    if (!(this.height < 1 || this.width < 1)) {
                        var imgInstance = new fabric.Image(this, {
                            ...
                            width = this.width,
                            height = this.height;
                        canvas.setWidth(width), canvas.setHeight(height), _this.darkroom.image.remove(), _this.darkroom.image = imgInstance, canvas.add(imgInstance), darkroom.dispatchEvent("image:change")
                    }
