...
var contact = $$.m.ModelBase.extend({
  validateData: function(data) {
    ...
    emailConstraints = {email: {presence: true}};
    if (data.details.length && data.details.emails && data.details.emails.length) {
      data.details.emails.forEach(function(value, index) {
        validate(value, emailConstraints);
      });
    }
  ...
  getEmails: function() {
    ...
    getPrimaryAddress: function() {
		...
		var address = _.findWhere(addresses, {defaultBilling: true});
  ...
  createOrUpdateDetails: function(type, source, socialId, photoMedium, photoLarge, photoSquare, emails, websites) {
    ...
    if (photoMedium != null) {
      photos.photoMedium = photoMedium;
    }
    if (photoLarge != null) {
      photos.photoLarge = photoLarge;
    }
    if (photoSquare != null) {
      photos.photoSquare = photoSquare;
    }
