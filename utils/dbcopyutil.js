...
var copyutil = {
    ...
    purgePingdom: function(fn) {
        ...
        async.waterfall([
            ...
            function(sessionIDAry, cb) {
                pingEventsCollection.remove({'session_id':{$in:sessionIDAry}}, function(err){
                    if(err) {
                        console.log('error removing pingEvents:', err);
                    } else {
                        console.log('removed pingEvents');
                    }
                    cb(null, sessionIDAry);
                });
            },
            function(sessionIDAry, cb) {
                sessionEventsCollection.remove({'session_id':{$in:sessionIDAry}}, function(err){
                    if(err) {
                        console.log('error removing sessionEvents:', err);
                    } else {
                        console.log('removed sessionEvents');
                    }
                    cb(null, sessionIDAry);
                });
            }
