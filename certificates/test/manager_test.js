...
module.exports.group = {
    ...
    testAddDomain: function(test) {
        ...
        fs.readFile('./prod_star_indigenous_io.key', 'utf-8', function(err, value){
            if(err) {
                test.ok(false, 'error reading file:' + err);
                test.done();
            } else {
                self.log.debug('\n'+value);
                var domains = ['www.fitstop-lab.com', 'www.sdjgfoundation.org'];
                manager.generateCSR(value, domains, function(err, csr){
                    self.log.debug('\n' + csr);
                    test.ok(true);
                    test.done();
                });
